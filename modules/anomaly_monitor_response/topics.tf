...
resource "aws_sns_topic_subscription" "team_spend_alert_email_subscription" {
  ...
  topic_arn = "arn:aws:sns:${var.AWS_REGION}:${var.awsID}:team-spend-alert"
  protocol  = "email"
  endpoint  = each.value
}
...
resource "aws_sns_topic_subscription" "team_spend_action_email_subscription" {
  for_each = {for email in var.team_spend_emails: email => email}
  topic_arn = "arn:aws:sns:${var.AWS_REGION}:${var.awsID}:team-spend-action"
  protocol  = "email"
  endpoint  = each.value
}
...
resource "aws_sns_topic_subscription" "team_spend_action_lambda_subscription" {
  topic_arn = "arn:aws:sns:${var.AWS_REGION}:${var.awsID}:team-spend-action"
  protocol  = "lambda"
  endpoint  = "arn:aws:lambda:${var.AWS_REGION}:${var.awsID}:function:team-spend-action"
}
